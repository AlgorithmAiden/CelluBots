...
document.getElementById('scanButton').onclick = () => { scanFilesInDirectory(); }
    ...
    try {
        ...
        for await (const entry of directoryHandle.values()) {
            if (entry.kind === 'file') {
                ...
                // You can get a file handle to read the file here
            } else if (entry.kind === 'directory') {
                console.log(`Directory: ${entry.name}`);
                // Optionally handle directories, possibly with recursion
            }
        }

        alert("Finished scanning files!");
    } catch (error) {
        console.error("Error accessing file system:", error);
        alert("An error occurred while scanning the directory.");
    }
}
